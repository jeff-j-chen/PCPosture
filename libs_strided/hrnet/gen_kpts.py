...
from libs_strided.hrnet.lib.utils.utilitys import plot_keypoint, PreProcess, write, load_json
from libs_strided.hrnet.lib.config import cfg, update_config
from libs_strided.hrnet.lib.utils.transforms import *
...
from libs_strided.yolov3.human_detector import yolo_human_det as yolo_det
from libs_strided.sort.sort import Sort
...
def gen_frame_kpts(frame, det_dim=416, num_people=1, gen_output=False):
    ...
    if people_track.shape[0] == 1:
        ...
        people_track_ = people_track[-num_people:, :-1].reshape(num_people, 4)
    ...
    with torch.no_grad():
        ...
        inputs, origin_img, center, scale = PreProcess(frame, track_bboxs, cfg, num_people)
    ...
    kpts = np.zeros((num_people, 17, 2), dtype=np.float32)
    scores = np.zeros((num_people, 17), dtype=np.float32)
